...
class ListCancelledSalesScreen extends StatefulWidget {
  const ListCancelledSalesScreen({super.key});

  @override
  State<ListCancelledSalesScreen> createState() => _ListCancelledSalesScreenState();
}

class _ListCancelledSalesScreenState extends State<ListCancelledSalesScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (!_didInit) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        prov.fetchAll(includeCancelled: true);
      });
      _didInit = true;
    }
  }
  ...
  void _onQueryChanged(String q) {
    ...
    _debounce = Timer(const Duration(milliseconds: _debounceMs), () {
      prov.fetchAll(q: q, includeCancelled: true);
    });
  }
  ...
  void _clearQuery() {
    _debounce?.cancel();
    _qCtrl.clear();
    prov.fetchAll(q: '', includeCancelled: true);
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Filtrar solo canceladas (status == false)
    final cancelled = sales.items.where((s) => s.status == false).toList();

    return Scaffold(
      appBar: AppBar(
        backgroundColor: primary,
        title: const Text('Ventas canceladas'),
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh),
            tooltip: 'Refrescar',
            onPressed: () => prov.fetchAll(q: _qCtrl.text, includeCancelled: true),
          ),
        ],
      ),
      body: Column(
        children: [
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onSubmitted: (v) => prov.fetchAll(q: v, includeCancelled: true),
          ...
          // Lista con scroll (solo ver, sin acciones)
          Expanded(
            child: sales.loading
                ...
                : (sales.error != null)
                    ...
                    : (cancelled.isEmpty)
                        ? const Center(child: Text('No hay ventas canceladas'))
                        : Scrollbar(
                                ...
                                return InkWell(
                                  onTap: () {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (_) => SaleDetailScreen(saleId: s.saleId),
                                      ),
                                    );
                                  },
                                  child: Container(
